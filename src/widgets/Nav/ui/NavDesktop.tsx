...
export default function NavDesktop({ className, ...props }: NavDesktopProps) {
  return (
    <nav className={cn("mx-auto max-w-screen-xl w-full px-3 items-center nav-height flex", className)} {...props}>
      ...
      <div className="grow flex items-center justify-center gap-16">
        {linksList.map((elem, index) => (
          <Link key={index} href={elem.href} className="text-slate-700 font-medium text-lg hover:text-slate-500">
            {elem.label}
          </Link>
        ))}
      </div>
      <Button asChild>
        <Link href={"/u/dashboard"}>Find My Care</Link>
      </Button>
    </nav>
  );
}
